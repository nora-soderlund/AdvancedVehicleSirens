...
std::map<int, bool> VehicleFoglights::states;

std::map<int, std::map<VehicleFoglightState, std::vector<VehicleMaterial*>>> VehicleFoglights::materials;
std::map<int, std::map<VehicleFoglightState, std::vector<VehicleDummy*>>> VehicleFoglights::dummies;

void VehicleFoglights::RegisterEvents() {
	...
	if (!PluginConfig::Lights->FogEnabled)
		return;
	...
	PluginKeys::Register(PluginConfig::Keys->Foglights, (PluginKeyFunction)[](int key, CVehicle* vehicle) {
		int model = vehicle->m_nModelIndex;
	...
	VehicleMaterials::RegisterRender((VehicleMaterialRender)[](CVehicle* vehicle, int index) {
		...
		if (PluginConfig::Lights->FogEnableWithSiren == false || vehicle->m_nVehicleFlags.bSirenOrAlarm == false) {
			int index = CPools::ms_pVehiclePool->GetIndex(vehicle);
